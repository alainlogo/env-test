...
$(document).ready(function(){
	$('.tbcmsbrandlist .tbcmsdelete').click(function(e){
		e.preventDefault();
	...
	function removeTags(){
    	setTimeout(function() {
          $(".tbcmsbrandlist-position-update").remove();
        }, 3000);
    }

	var url = baseDir + "modules/tbcmsbrandlist/ajax.php";
    var obj = $(document).find('body');
    $(".tbcmsbrandlist tbody").sortable({
        opacity: 0.6,
        cursor: 'move',
        update: function() {
            ...
            $.post(url, order, function(data) {
                ...
                if (result == 'right') {
                    var tags = '';
                    tags += '<div id="growls" class="default tbcmsbrandlist-position-update"><div class="growl growl-notice growl-medium">';
