...
<table id="table-data" class="table tbcmstable tbcmsbrandlist">
	<thead>
		<tr class="nodrag nodrop">
			{if $show_fields.image == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Image' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}

			{if $show_fields.title == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Title' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}
			...
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Short Description' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}

			{if $show_fields.description == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Description' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}

			{if $show_fields.link == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Link' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}

			{if $show_fields.status == true}
			<th class="fixed-width-xs center status">
				<span class="title_box active">{l s='Status' mod='tbcmsbrandlist'}</span>
			</th>
			{/if}

			<th class="fixed-width-xs center action">
				<span class="title_box active">{l s='Action' mod='tbcmsbrandlist'}</span>
			</th>
		</tr>	
	</thead>
			...
			<tr id="recordsArray_{$Data.id|escape:'htmlall':'UTF-8'}">
				...
				<td class="pointer fixed-width-xs center">
					<form action="" class="actionEdit" name="actionEdit" method="post">
						...
						<button title="Edit" class="edit" onclick="$(this).parent('.actionEdit').trigger('submit');" ><i class="icon-pencil"></i> {l s='Edit' mod='tbcmsbrandlist'} </button>						
					</form>
					<form  action="" class="actionDelete" name="actionDelete" method="post"> 
						<input type="hidden" name="id" value="{$Data.id|escape:'htmlall':'UTF-8'}">		
						<input type="hidden" name="action" value="remove">		
						<button title="Delete" class="delete tbcmsdelete" ><i class="icon-trash"></i> {l s='Delete' mod='tbcmsbrandlist'} </button>
					</form>
				</td>
			</tr>
		{/foreach}
