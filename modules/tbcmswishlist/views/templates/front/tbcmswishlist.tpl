...
<div id="wishlist_block" class="block account">
	<h4 class="title_block">
		<a href="{$link->getModuleLink('tbcmswishlist', 'mywishlist', array(), true)|addslashes|escape:'htmlall':'UTF-8'}" title="{l s='My wishlists' mod='tbcmswishlist'}" rel="nofollow">{l s='Wishlist' mod='tbcmswishlist'}</a>
	</h4>
	<div class="block_content">
		...
		{if $wishlist_products}
			...
			{foreach from=$wishlist_products item=product name=i}
				<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product|escape:'htmlall':'UTF-8'}', {$product.id_product_attribute|escape:'htmlall':'UTF-8'}, '0', '{if isset($token)}{$token|escape:'htmlall':'UTF-8'}{/if}');" title="{l s='remove this product from my wishlist' mod='tbcmswishlist'}" rel="nofollow"><img src="{$img_dir|escape:'htmlall':'UTF-8'}icon/delete.gif" width="12" height="12" alt="{l s='Delete' mod='tbcmswishlist'}" class="icon" /></a>
				</dt>
				{if isset($product.attributes_small)}
				<dd class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					<a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category_rewrite)|escape:'html'}" title="{l s='Product detail' mod='tbcmswishlist'}">{$product.attributes_small|escape:'html':'UTF-8'}</a>
				</dd>
				{/if}
			{/foreach}
			</dl>
		{else}
			<dl class="products">
				<dt>{l s='No products' mod='tbcmswishlist'}</dt>
			</dl>
		{/if}
		</div>
		<p class="lnk">
		...
		{/if}
			<a href="{$link->getModuleLink('tbcmswishlist', 'mywishlist', array(), true)|addslashes|escape:'htmlall':'UTF-8'}" title="{l s='My wishlists' mod='tbcmswishlist'}" rel="nofollow">&raquo; {l s='My wishlists' mod='tbcmswishlist'}</a>
		</p>
	</div>
</div>
{/strip}