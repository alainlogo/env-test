...
$(document).ready(function(){
	$('.tbcmspaymenticon .tbcmsdelete').click(function(e){
		e.preventDefault();
	...
	function removeTags(){
    	setTimeout(function() {
          $(".tbcmspaymenticon-position-update").remove();
        }, 3000);
    }

	var url = baseDir + "modules/tbcmspaymenticon/ajax.php";
    var obj = $(document).find('body');
    $(".tbcmspaymenticon tbody").sortable({
        opacity: 0.6,
        cursor: 'move',
        update: function() {
            ...
            $.post(url, order, function(data) {
                ...
                if (result == 'right') {
                    var tags = '';
                    tags += '<div id="growls" class="default tbcmspaymenticon-position-update"><div class="growl growl-notice growl-medium">';
