...
<div id="view_wishlist">
    <div class="tbwilshlist-view-title page-heading bottom-indent">
        {l s='Wishlist' mod='tbcmswishlist'}
    </div>
    {if $wishlists}
        <p class="tbwishlist-view-page-first-dec">
            <strong class="dark">
                {l s='Other wishlists of %1s %2s:' sprintf=[$current_wishlist.firstname, $current_wishlist.lastname] mod='tbcmswishlist'}
            </strong>
            {foreach from=$wishlists item=wishlist name=i}
                {if $wishlist.id_wishlist != $current_wishlist.id_wishlist}
                    <a href="{$link->getModuleLink('tbcmswishlist', 'view', ['token' => $wishlist.token])|escape:'html':'UTF-8'}" rel="nofollow" title="{$wishlist.name|escape:'html':'UTF-8'}">
        ...
        <ul class=" wlp_bought_list">
            {foreach from=$products item=product name=i}
                ...
                <li id="wlp_{$product.id_product|escape:'htmlall':'UTF-8'}_{$product.id_product_attribute|escape:'htmlall':'UTF-8'}" class="ajax_block_product col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 {if $smarty.foreach.i.iteration%$nbItemsPerLine == 0} last-in-line{elseif $smarty.foreach.i.iteration%$nbItemsPerLine == 1} first-in-line{/if} {if $smarty.foreach.i.iteration > ($smarty.foreach.i.total - $totModulo)}last-line{/if} {if $smarty.foreach.i.iteration%$nbItemsPerLineTablet == 0}last-item-of-tablet-line{elseif $smarty.foreach.i.iteration%$nbItemsPerLineTablet == 1}first-item-of-tablet-line{/if} {if $smarty.foreach.i.iteration > ($smarty.foreach.i.total - $totModuloTablet)}last-tablet-line{/if}">
                    <div class="tbwishlist-view-all-block-content">
                        <div class="tbwishlist-product-view-img-block">
                        ...
                        <div class="tbwishlist-view-product-info-box">
                            <div class="product_infos">
                                ...
                                <div class="wishlist_product_detail">
                                    ...
                                    <p class="form-group selector1">
                                        <span><strong>{l s='Priority' mod='tbcmswishlist'}:</strong> {$product.priority_name|escape:'htmlall':'UTF-8'}</span>
                                    </p>
                                        ...
                                        {if (isset($product.attribute_quantity) && $product.attribute_quantity >= 1) || (!isset($product.attribute_quantity) && $product.product_quantity >= 1) || (isset($product.allow_oosp) && $product.allow_oosp)}
                                                <form action="{$urls.pages.cart|escape:'htmlall':'UTF-8'}" method="post">
                                                    ...
                                                    <button data-button-action="add-to-cart" class="btn btn-primary">
                                                        {l s='Add To Cart' mod='tbcmswishlist'}
                                                        <i class="fa fa-shopping-cart"></i>
                                                    </button>
                                                </form>
                                        {else}
                                            <span class="button ajax_add_to_cart_button btn btn-default disabled">
                                                <span>{l s='Add to cart' mod='tbcmswishlist'}</span>
                                            </span>
                                        {/if}
                                        <a class="button lnk_view btn btn-primary btn-default tbwishlist-view-product-view-btn" href="{$link->getProductLink($product.id_product,  $product.link_rewrite, $product.category_rewrite)|escape:'html':'UTF-8'}" title="{l s='View' mod='tbcmswishlist'}" rel="nofollow">
                                            <span>{l s='View' mod='tbcmswishlist'}</span>
                                        </a>
                                    </div>
                                    <!-- .btn_action -->
                                </div>
